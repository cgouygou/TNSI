...
!!! example "`list` vs. `dict` : comparaison des temps d'exécution d'une recherche"
    === "Énoncé" 
        ...
        On crée donc deux fonctions qui:

        - construit une liste d'entiers de taille `N` pour `timeit_lst` 
        - construit un dictionnaire  de taille `N` dont les clés sont des entiers pour `timeit_dct`
        - puis qui y recherche ensuite le `str` `"a"`... un certain nombre de fois (`nbloop`) en mesurant le temps d'exécution à l'aide de la fonction `time` du module `time` pour obtenir une moyenne.
