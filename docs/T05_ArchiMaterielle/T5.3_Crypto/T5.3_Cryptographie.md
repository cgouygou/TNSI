...
!!! abstract "Principe d'un chiffrement"
    ...
    Cette clé peut être la même pour chiffrer et déchiffrer (ou se déduire directement) : on parle alors de **chiffrement  symétrique**. Si la clé de chiffrement et la clé de déchiffrement sont différentes, on parle de **chiffrement asymétrique**.
...
!!! example "{{ exercice() }}: chiffre de César"
    === "Énoncé" 
        ...
        2. Avec quelle clé déchiffre-t-on un message chiffré avec une clé égale à 10? 
        2. Compléter le code suivant (utiliser les fonctions `#!py ord` et `#!py chr` pour la fonction `#!py decale`):
            ...
            ```
        4. Écrire un jeu de tests (avec `#!py assert`) pour tester vos fonctions.

        5. Déchiffrer le message suivant:

            `#!py 'PRZRFFNTRARPBAGVRAGEVRAQVAGRERFFNAGZNVFVYRFGFHSSVFNZRAGYBATCBHEDHRPRFBVGCRAVOYRQRYRQRPUVSSERENYNZNVA'` 

    === "Indications"
        1. J'espère que vous connaissez la longueur de l'alphabet.
        2. Il faut faire un tour complet de l'alphabet.
        3. On travaille sur des rangs dans l'alphabet (donc de 0 à ...). La fonction `#!py ord` renvoie le code Unicode d'un caractère. Comme tous les caractères sont dans l'ordre, il suffit de comparer le code d'un caractère (par exemple `#!py 'G'` a pour code `#!py 71`) avec celui de `#!py 'A'` (c'est-à-dire `#!py 65`) pour en déduire son rang (6 pour `#!py 'G')`.
        4. Débrouillez-vous.
        5. On procède par force brute (recherche exhaustive) en testant toutes les clés possibles (avec une boucle s'il vous plaît) et on lit chaque message pour trouver le bon.
...
!!! example "{{ exercice() }}: analyse de fréquences"
    === "Énoncé" 
        Le chiffre de César est un chiffrement *monoalphabétique* : chaque lettre/caractère est tout le temps chiffré par la même lettre/caractère.
        
        Pour déchiffrer, c'est-à-dire récupérer la clé, il suffit de procèder par [analyse de fréquences](https://fr.wikipedia.org/wiki/Analyse_fr%C3%A9quentielle){:target="_blank"}.
        Il suffit pour cela de parcourir le texte chiffré et de construire un dictionnaire associant à chaque caractère son nombre d'occurences (ça ne vous rappelle rien?). 
        On suppose ensuite que le caractère ayant le nombre maximal d'occurences correspond au caractère *a priori* le plus fréquent dans le texte clair, c'est-à-dire dans beaucoup de langues la lettre `#!py 'E'` 

        1. Parcourir le texte suivant et construire un dictionnaire dont les clés sont les caractères apparaissant dans le texte et les valeurs leur nombre d'occurences.
        2. Récupérer le caractère ayant le plus d'occurences, et en déduire la clé (le texte est chiffré avec le chiffre de César).
        3. Déchiffrer.
...
!!! example "{{ exercice() }}: masque jetable ou chiffre de Vernam"
    ...
    === "Indications"
        Si vous vous souvenez du DL0001, la clé de déchiffrement est identique à celle de chiffrement, puisque deux «xor» successifs par la même valeur s'annulent.
    === "Correction" 
        {{ correction(False, 
        "
        ```python
...
!!! example "{{ exercice() }}: Chiffrement affine"
    ...
    === "Indications"
        - **Rappel** : par exemple, 7 et 12 sont premiers entre eux car ils n'ont aucun diviseur commun autre que 1 (leur PGCD est 1), mais 8 et 12 ne sont pas premiers entre eux car leur PGCD est 4.
        - Le PGCD s'obtient par la fonction `#!py gcd` du module `#!py math`.
        - Mathématiquement, vous n'avez pas (encore) les connaissances pour inverser la clé. On se contente donc pour déchiffrer de chiffrer avec des clés quelconques et de vérifier si les messages chiffrés correspondent.
...
!!! pydefi "Si vous avez fini..."
    - Essayez de coder une fonction qui chiffre selon la méthode de Vigenère (cf. lien en introduction).
    - [Code Konami](https://pydefis.callicode.fr/defis/C22_KonamiCode/txt){:target="_blank"} (demandez de l'aide à Basit).
