...
## 4. Algorithme de Douglas-Peuker

Une ligne polygonale est constituée d’une liste ordonnée de points, appelés sommets,
joints par des segments. L’algorithme de Douglas-Peuker permet de simplifier une ligne
polygonale en supprimant certains de ses sommets.

L’effet de l’algorithme appliqué aux
lignes polygonales du contour de la France métropolitaine est illustré ci-dessous.

![](../images/DP_france.png){: .center} 


!!! gear "Principe de l'algorithme"
    L’algorithme de Douglas-Peuker repose sur une stratégie de type « diviser pour régner ».

    Étant donné une ligne polygonale, c'est-à-dire une liste de points, l'objectif est d'éliminer des points «proches de l'alignement». Pour définir cette notion de *proche*, un seuil (une valeur numérique) est fixée.


    Le principe de l'algorithme est le suivant:

    - si la ligne ne contient qu’un ou deux sommets, l’algorithme se termine ;
    - sinon, on considère la droite formée par les extrémités de la ligne (son premier et
    dernier sommet), et on sélectionne le point le plus éloigné de cette droite:

        - si la distance entre le point sélectionné et la droite est inférieure au seuil fixé, on ne conserve que les extrémités
        de la ligne polygonale :

        ![](../images/DP_algo1.png){: .center} 

        - sinon, on applique l’algorithme de manière récursive aux deux parties de la ligne
        polygonale formées de la séquence formée du premier sommet jusqu’au
        sommet sélectionné d’une part et de la séquence formée du sommet
        sélectionné jusqu’au dernier sommet d’autre part. L’algorithme renvoie alors la
        *concaténation* des séquences simplifiées ainsi obtenues.

        ![](../images/DP_algo2.png){: .center} 

    
    Par exemple, l'algorithme appelé sur la ligne polygonale [A,B,C,D,E,F,G] ci-dessus, va
    récursivement être appelé sur les lignes polygonales [A,B,C] et [C,D,E,F,G].
    La ligne polygonale que l'on obtiendra à la fin de l'algorithme sera [A,C,E,G]:

    ![](../images/DP_algo3.png){: .center} 


Avant d'implémenter cet algorithme, nous allons d'abord implémenter des fonctions auxiliaires.
...
!!! code "Distance entre deux points"
    La distance qui sépare deux points A et B de coordonnées $(x_A; y_A)$ et $(x_B; y_B)$ est donnée par la formule $\sqrt{(x_B-x_A)^2+(y_B-y_A)^2}$.

    On rappelle que la fonction `#!py sqrt` du module `#!py math` renvoie la racine carrée d'un nombre positif ou nul.

    Écrire une fonction `#!py distance_points` qui prend en paramètre deux tuples représentant les coordonnées de deux points et qui renvoie la distance qui les sépare.
...
!!! code "Distance entre un point et une droite"
    La fonction `#!py distance_point_droite` ci-dessous prend en paramètre les tuples représentant les coordonnées respectives des points P, A et B et renvoie la distance du point P à la droite (AB). L'exécution de cette fonction produit une erreur dans le cas où les points A et B sont égaux.
    ...
    À l'aide des fonctions `#!py distance_points`  et `#!py distance_point_droite`, écrire une fonction `#!py distance(p,a,b)`  qui renvoie la distance entre le point P et la
    droite (AB) si les points A et B sont distincts et la distance AP sinon.
...
!!! code "Point le plus éloigné"
    On a maintenant besoin d'une fonction `#!py le_plus_loin` qui prend en paramètre une liste de tuples représentant les coordonnées des points d’une ligne polygonale.

    Cette fonction doit renvoyer un tuple composé de :
    - l’indice du point de coordonnées `p` de la ligne polygonale d’extrémités `deb` et `fin`,
    pour lequel la distance `distance(p, deb, fin)` est la plus grande ;
    - la valeur correspondante de cette distance.

    Compléter le code suivant:

    ```python linenums='1'
    def le_plus_loin(ligne:lst) -> tuple:
        n = len(ligne)
        deb = ligne[...]
        fin = ligne[...]
        dmax = 0
...
!!! code "No slicing in NSI"
    L'algorithme de Douglas-Peuker va être rappelé récursivement sur des lignes polygonales plus courtes. Il faut donc pouvoir extraire d'une liste des éléments compris entre un indice i (**inclus**) et j (**inclus**) avec bien entendu $0 \leqslant i \leqslant j < \text{len(tab)}$.

    Compléter cette fonction.

    ```python linenums='1'
    def extrait(tab:list, i:int, j:int) -> list:
        return [ ... for k in range(..., ...)]
    ```

    **Tester son code:**
    ```python
    >>> extrait([3, 8, 2, 7, 5, 1], 2, 4)
    [2, 7, 5]
    ```
    
!!! code "Algorithme de Douglas-Peuker"
    L’algorithme de Douglas-Peuker est implémenté par la fonction `#!py simplifie`  ci-dessous, qui prend en paramètre la ligne polygonale et le seuil choisi.

    **Rappel:** dans le cas où le point «le plus loin» est à une distance supérieure au seuil, la fonction renvoie la concaténation des simplifications (c'est-à-dire une somme de listes) sur des extraits de la ligne polygonale.
...
??? gear "Application sur la carte de France"
    Télécharger d'abord le fichier [coord_france.txt](../data/coord_france.txt){:target="_blank"} dans le même répertoire que votre fichier.

    Adaptez votre code ainsi:
    ...
    Modifier la valeur du seuil passée en argument pour voir l'effet de l'algorithme.


## 5. Exercices
{{ initexo(0) }}

!!! example "{{ exercice() }} (BAC)"
    === "Énoncé" 
        Cet exercice est composé de trois parties indépendantes.

        Dans cet exercice, on s’intéresse à des algorithmes pour déterminer, s’il existe,
        l’élément absolument majoritaire d’une liste.

        On dit qu’un élément est absolument majoritaire s’il apparaît dans strictement plus de
        la moitié des emplacements de la liste.

        Par exemple, la liste `#!py [1, 4, 1, 6, 1, 7, 2, 1, 1]`  admet 1 comme élément
        absolument majoritaire, car il apparaît 5 fois sur 9 éléments. Par ailleurs, la liste `#!py [1, 4, 6, 1, 7, 2, 1, 1]`  n’admet pas d’élément absolument majoritaire, car celui qui est le plus fréquent est 1, mais il n’apparaît que 4 fois sur 8, ce qui ne fait pas **plus** que la moitié.
