...
??? code "Partie 1: Mise en place"
    ...
    - une classe `Labyrinthe`  qui contient un unique attribut `grille` de type `list` qui contient des celluleset dont il faut compléter la méthode `construire_grille`.
