...
!!! example "{{ exercice() }}"
    === "Énoncé" 
        Questions 1 à 6 de l'**exercice 2** du sujet BAC ci-dessous.

        <span class='centre'>
        [Polynésie 2024 Jour 1 :material-download:](../data/24NSIJ1PO1.pdf){.center .md-button }
        </span>

        Mettre en œuvre la solution donnée à la question 6.
    ...
    === "Correction" 
        ...
        def nb_solutions_memo(n):
            ...
            def nbs(n):
                ...
                if n in memo:
                    return memo[n]
                memo[n] = nbs(n-3) + nbs(n-5) + nbs(n-7)
                return memo[n]
            return nbs(n)
        ```
